...
public final class CompositeDisposable {
    ...
    public var isDisposed: Bool {
        return self.queue.sync { _isDisposed }
    }
    ...
    public func add(_ disposable: Disposable) {
        let toDispose: Disposable? = self.queue.sync(flags: .barrier) {
            guard !self._isDisposed else { return disposable }
    ...
    public func add(_ disposables: [Disposable]) {
        let toDispose: [Disposable]? = self.queue.sync(flags: .barrier) {
            guard !self._isDisposed else { return disposables }
    ...
    /// - Remark: Thread-safe
    public func dispose() {
        let currentDisposables: Set<Disposable>? = self.queue.sync(flags: .barrier) {
            let currentDisposables = self.disposables
