...
public final class Store<State> {
    ...
    public var state: State {
        return self.queue.read { currentState }
    }
    ...
    public init(id: String = "swifty-redux.store", state: State, reducer: @escaping Reducer<State>, middleware: [Middleware<State>] = []) {
        ...
        (self.observable, self.observer) = Observable<State>.pipe(id: "\(id).observable")

        self.dispatchFunction = applyMiddleware(middleware)(
    ...
    public func subscribe(on queue: DispatchQueue? = nil,
                          ...
                          observer: @escaping (_ state: State) -> Void) -> Disposable
    {
        ...
        return self.observable.subscribe(observer: observer)
    }
}
